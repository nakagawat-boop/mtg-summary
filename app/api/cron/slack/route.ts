...
function diff(now: number, prev: number, unit: string) {
  const d = now - prev;
...
export async function GET(req: Request) {
  ...
  const scRows = scR.rows || [];
  const csRows = csR.rows || [];
  const scThis = scRows[0]?.ca || [];
  const scPrev = scRows[1]?.ca || [];
  const csThis = csRows[0]?.ca || [];
  const csPrev = csRows[1]?.ca || [];
